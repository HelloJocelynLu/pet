...
# Generates a new state, give it a name
    ...
    def getage(self, time):
        ...
        self.message.append(self.text("age").replace(
            "%t", str(int(ageindays))))
    ...
    def testaction(self, time, lasttime, forcestop=False):
        ...
        timereq = self.settings.config[self.state["action"] +
                                       "time"] * self.settings.config["timemodifier"]
        ...
        if timediff >= timereq or forcestop:
            if self.state["action"] == "heal" and not forcestop:
                ...
                self.state[self.state["action"]] = self.state[self.state["action"]] + \
                    (self.settings.config[self.state["action"] +
                                          "improve"]*percent)
                self.effect(percent)
        ...
        if not self.state["dead"]:
            for key in ["food", "sleep", "clean", "play", "learn"]:
                ...
                self.state[key] = self.state[key]-(
                    (timediff / self.settings.config["timemodifier"]) * self.settings.config[key+"mod"])
    ...
    def effect(self, percent):
        for key in ["food", "sleep", "clean", "play", "learn"]:
            ...
            if effectkey in self.settings.config:
                self.state[key] = self.state[key] - \
                    (self.settings.config[effectkey]*percent)
            effectkey = self.state["action"]+"+"+key
            if effectkey in self.settings.config:
                self.state[key] = self.state[key] + \
                    (self.settings.config[effectkey]*percent)
